...
This directory contains automatically generated performance charts based on benchmark data from `benchmarking/results/`.
...
```bash
# Generate modern charts from benchmarking/results data only
...
### New System (benchmarking/results only)
- `benchmarking/results/jmh-*.json` - JMH microbenchmark results with detailed statistics
- `benchmarking/results/macro-*.json` - Macro benchmark results with execution metrics

### Legacy System (Multiple sources - deprecated)
- `rmatch-tester/logs/*.csv` - Historical CSV data
- `benchmarking/results/performance-check-*.json` - Performance check reports (not currently generated)
