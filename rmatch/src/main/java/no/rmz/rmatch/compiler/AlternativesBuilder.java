...
public final class AlternativesBuilder {
  ...
  /**
   * This will be the result of the compilation. The actual result returned by the getResult method
   * is the arrival node of the first CompiledFeragment.
   */
  private final LinkedList<CompiledFragment> alternatives = new LinkedList<>();

  /** A reference to the last fragment added, or null if we are parsing a new alternative. */
  ...
  public CompiledFragment build() {
    checkState(!hasBeenBuilt, "Attempt to build an alrady built AlternativesBuilder");
  ...
  /**
   * When starting a new alternative, the old sequence of patterns must be wrapped into a
   * CompiledFragment representing the alternative. This method does that by performing a bit of
   * surgery on the list.
   */
  private void wrapSequenceOfFragmentIntoFragment() {
    if (lastFragment != null) {
      ...
      alternative.getArrivalNode().addEpsilonEdge(alternativeHead.getArrivalNode());
      lastFragment.getEndingNode().addEpsilonEdge(alternative.getEndingNode());
  ...
  /**
   * Add a new fragment to the set of alternatives. Add it to the current alternative being built,
   * or create a new one if instructed to by having the newAlternative flag set.
   *
   * @param fragment fragment to add after the last one.
   */
  public void addLast(final CompiledFragment fragment) {

    checkState(!hasBeenBuilt, "Attempt to add more to an already built AlternativesBuilder");
    if (newAlternative) {
      wrapSequenceOfFragmentIntoFragment();
      ...
      lastFragment.getEndingNode().addEpsilonEdge(fragment.getArrivalNode());
    }
  ...
  public CompiledFragment getLastFragment() {
    checkState(
        !hasBeenBuilt, "Attempt extract something from " + "an already built AlternativesBuilder");
    return lastFragment;
  }

  /**
   * When separating alternatives, this method is called to make sure that the next time addLast is
   * added, the next element is added to a new alternative.
   */
