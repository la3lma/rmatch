...
public final class RegexpStorageTest {
  ...
  /**
   * A set of strings that is used when checking that the strings representing regexps in the
   * RegexpStorage contains a specific set of regexps strings, i.e. the set wee have put into the
   * storage.
   */
  private Set<String> stringSet;

  /**
   * A mocked compiler. The behavior is mocked up to return the compilationResult node as the result
   * of any compilation.
   */
  @Mock NDFACompiler compiler;

  /**
   * Placeholder mock. Not used for anything as far as the tests are concerned (but necessary to
   * obey contracts with interfaces).
   */
  @Mock NodeStorage storage;

  /** The compilation result. Just mocked, treated as an opaque object. */
  @Mock NDFANode compilationResult;

  /** A mocked action, only treated as an opaque object. */
  @Mock Action a;

  /**
   * Set up, will poulate the variables reString, stringSet and rs, as well as mocking up the
   * behavior of the compile instance to return the content compilationResult whenever the compile
   * method is invoked.
   */
  @BeforeEach
  ...
  /** Checking that all the regexps that are put into the storage is actually there. */
  @Test
