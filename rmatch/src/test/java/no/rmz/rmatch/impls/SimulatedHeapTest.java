...
public final class SimulatedHeapTest {
  ...
  /** An array of integers, with the property that i[j] = j and 0 <= j <= maxi. */
  private Integer[] i;
  ...
  /** Set u the array "i" to contain integers i[j] = j for j in the range 0 ... 99. */
  @BeforeEach
  public void setUp() {
  ...
  /**
   * Add two elements in reversed order (forst second then first) and see that the elements that are
   * removed elements are in fact first the first and then the second.
   */
  @Test
  public void test2() {
    assertTrue(h.isEmpty());
  ...
  /**
   * Add elements in a pseudo-random order (addition of 73 modulo maxi). then get/remove all
   * elements, and they had better come out in the right order ;)
   */
  @Test
