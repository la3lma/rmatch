...
class NodeStorageImplTest {
  ...
  /** An implementation instance. This is the test article that we check the response of. */
  private NodeStorageImpl nsi;

  /**
   * A mocked NDFANode that will be used in various ways. The tests will set up this variable for
   * themselves.
   */
  ...
  /**
   * Check any invariants that should be true for the globally declared data structures are actually
   * true post ante.
   */
  @AfterEach
  public final void tearDown() {
    assertTrue(emptyNdfaNodeSet.isEmpty(), "Expecting emptyNdfaNodeSet to be empty");
  }
  ...
  /**
   * A corner case: to test is where we have an empty set of NDFANOdes, do we get the same DFA when
   * we look for it twice (with different empty set instances)?
   */
  @Test
  ...
  /**
   * Mock a regexp. We won't ever look into this regexp it's just treated as an opaque object as far
   * as the tests are concerned.
   */
  @Mock Regexp regexp;

  /**
   * If we have a node based on a single NDFA, do we get the same DFA node when looking for it
   * twice?
   */
  @Test
  ...
  /**
   * A class that will accept all sequences of all characters. This is essntially an implementation
  ...
  /**
   * We now set up three NDFANodes based the three size two subsets of a set of size three and make
   * sure that we get the right DFANOdes for many kinds of lookups using these combinations.
   */
  @Test
