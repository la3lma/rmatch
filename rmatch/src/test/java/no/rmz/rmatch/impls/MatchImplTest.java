...
public class MatchImplTest {
  ...
  /** A mocked match set, used to put the match in a familiar context. */
  @Mock MatchSet ms;
  ...
  /** A mocked regexp. */
  @Mock Regexp regexpMocked;

  /** A holder of runnable matches, mocked out. */
  @Mock RunnableMatchesHolder mockRunnableMatches;

  /** A test article. */
  private Match match;

  /** A real implementation, but the components it contains are mocked. */
  ...
  /**
   * Check that when comparing a match by domination to null, a null pointer exception is thrown.
   */
  @Test
  public final void testCompareToNull() {
    NullPointerException thrown =
        assertThrows(
            NullPointerException.class, () -> Match.COMPARE_BY_DOMINATION.compare(match, null));
  }

  /** Check that two presumably equal matches are indeed equal when compared using domination. */
  @Test
  ...
  /**
   * A final and inactive match will when it is committed be sent to a set of runnable matches.
   *
   * <p>XXX The negation isn't tested!
   */
  @Test
  ...
  public final void testCommitNotDominatingButStronglyDominatingOnFinalMatch() {
    ...
    when(regexpMocked.isStronglyDominated(semiMockedMatch)).thenReturn(true);
    MatchSetImpl.commitMatch(match, mockRunnableMatches);
