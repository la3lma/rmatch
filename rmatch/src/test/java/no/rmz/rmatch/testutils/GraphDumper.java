...
public final class GraphDumper {
  ...
  /** Our log. */
  private static final Logger LOG = Logger.getLogger(GraphDumper.class.getName());
  ...
  /**
   * True iff the DFANOde in question is terminal for any regexp.
   *
  ...
  /**
   * Utility method to get an unique print representation for the name of an NDFA node.
   *
   * @param node the node.
   ...
   */
  static void dumpNdfa(final Collection<NDFANode> ndfaNodes, final PrintStream out) {
  ...
  /**
   * Dump a representation of an epsilon edge that is parsable by GraphViz.
   *
  ...
  private static void printEpsilonEdge(
      final PrintStream out, final NDFANode source, final NDFANode destination) {
    checkNotNull(out);
    checkNotNull(source);
  ...
  /**
   * Print an edge with a label, e.g. "x" or "[a-b]", representing the character that must be
   * matches for this edge to be followed.
   *
  ...
  private static void printLabeledEdge(
    ...
    checkNotNull(destination);
    checkNotNull(label);
    out.printf("%s -> %s [label = \"%s\" ]; \n", nodeName(source), nodeName(destination), label);
  }

  /**
   * Dump the automata (determinstic and not) produced during the running of a test to a file in the
   * target/nodeDumps directory.
   *
   * @param nameOfTest Then name of the test.
   * @param nodeStorage The nodes to dump.
   */
  public static void dump(final String nameOfTest, final NodeStorage nodeStorage) {
    checkNotNull(nodeStorage);
    checkNotNull(nameOfTest);

    final String baseDumpDirectoryName = "target/nodeDumps/";
    final String dumpDiretoryName = baseDumpDirectoryName + nameOfTest + "/";
    ...
    if (!dumpDir.exists() || !dumpDir.isDirectory()) {
      if (!dumpDir.exists()) {
        ...
        if (!mkdirs) {
          throw new RuntimeException("Couldn't make directory: " + dumpDiretoryName);
        }
      } else {
  ...
  /**
   * Dump all nodes and the edges between them as graphviz representations to the "out" PrintStream.
   *
   * @param nodeStorage A set of nodes to dump.
   * @param ndfaOut the PrintSteram to dump a GraphViz representation to.
   */
  public static void dump(
      final NodeStorage nodeStorage, final PrintStream ndfaOut, final PrintStream dfaOut) {
    checkNotNull(nodeStorage);
  ...
  /**
   * Dump all the nodes, both dfa and ndfa, to a printsteam representing the nodes and the edges
   * between them as a GraphViz graph.
   *
  ...
  /**
   * Given a DFANode, generate a name for it that will be used as the nodename in the Graphviz
   * representation.
   *
   ...
   */
  private static void dumpDfa(final Collection<DFANode> dfaNodes, final PrintStream out) {
    checkNotNull(dfaNodes);
    checkNotNull(out);
    ...
    for (final DFANode source : dfaNodes) {
      final Map<Character, DFANode> nextMap = ((DFANodeImpl) source).getNextMap();
      for (final Entry<Character, DFANode> entry : nextMap.entrySet()) {

        out.printf(
            "%s -> %s [label = \"%c\" ]; \n",
            nodeName(source), nodeName(entry.getValue()), entry.getKey());
      }
    }
    out.println("}");
  }
