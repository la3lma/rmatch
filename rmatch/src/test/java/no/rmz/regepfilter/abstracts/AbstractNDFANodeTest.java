...
public class AbstractNDFANodeTest {
  ...
  /** The constructor should make a terminal node, so we check that it is. */
  @Test
  public final void testIsTerminal() {
    assertTrue(testArticleNDFANode.isTerminal());
  }

  /** Get the stored regexp and check that it's the one we put into it. */
  @Test
  ...
  /**
   * Check that when checking if the test article is active, that check involves asking the regexp
   * if it's active for the test article.
   */
  @Test
  ...
  /**
   * Check that when setting that a regexp is active for a node, then the node will be seen as
   * active.
   */
  @Test
  ...
  /** Check that the default (zero) number of epsilons is really what we're starting with. */
  @Test
  ...
  /** Test adding an epsilon edge, and checking that it was indeed added. */
  @Test
  ...
  /**
   * A really simple implementation of an abstract node. We need to make an implementation. Since
   * the AbstractNDFANode is, well, abstract, and can't be instantiated (yeah yeah, anonymous
   * classes etc.)
   */
  public static final class AbstractNDFANodeImpl extends AbstractNDFANode {
     ...
     */
    public AbstractNDFANodeImpl(final Regexp r, final boolean isTerminal) {
      super(r, isTerminal);
    }
