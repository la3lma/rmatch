...
final class MatchPairAnalysis {
  ...
  public MatchPairAnalysis(
    ...
    if (!javaResult.matcherTypeName().equals("java")) {
      System.err.println(
          "Java result parameter does not have matcher type java, was '"
    ...
    this.result =
        new MatcherBenchmarker.TestPairSummary(
            System.currentTimeMillis(),
            testSeriesId,
            metadata,
            rmatchResult.matcherTypeName(),
            rmatchResult.usedMemoryInMb(),
            rmatchResult.durationInMillis(),
            javaResult.matcherTypeName(),
            javaResult.usedMemoryInMb(),
  ...
  public void printSummary(PrintStream out) {
    ...
    out.println(
        "Sum of matches and mismatches = "
            + (numberOfMismatchesDetected + numberOfCorrespondingMatchesDetected));
    out.println(
        "Native to regexp ratio = "
            + (float) this.originalRmatchResult.durationInMillis()
                / (float) this.originalJavaResult.durationInMillis());
  }
