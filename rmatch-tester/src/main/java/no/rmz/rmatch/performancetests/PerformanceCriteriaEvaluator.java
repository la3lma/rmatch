...
public final class PerformanceCriteriaEvaluator {
    ...
    public static class PerformanceResult {
        ...
        public PerformanceResult(double timeImprovementPercent, double memoryImprovementPercent, 
                                Status status, String explanation, boolean statisticallySignificant) {
    ...
    public static PerformanceResult evaluate(
        ...
        if (currentResults.size() < 3) {
            return new PerformanceResult(0, 0, Status.FAIL, 
                "Insufficient runs: need minimum 3, got " + currentResults.size(), false);
        }
        
        if (baselineResults.isEmpty()) {
            return new PerformanceResult(0, 0, Status.WARNING,
                "No baseline data available for comparison", false);
        }
        ...
        double timeImprovementPercent = (baselineTimeStats.getMean() - currentTimeStats.getMean()) 
                                       / baselineTimeStats.getMean();
        double memoryImprovementPercent = (baselineMemoryStats.getMean() - currentMemoryStats.getMean()) 
                                         / baselineMemoryStats.getMean();

        // Check for statistical significance using coefficient of variation
        boolean statisticallySignificant = isStatisticallySignificant(currentTimeStats, baselineTimeStats);

        return determineStatus(timeImprovementPercent, memoryImprovementPercent, statisticallySignificant);
    }
    ...
    private static PerformanceResult determineStatus(double timeImprovementPercent, 
                                                   double memoryImprovementPercent,
                                                   boolean statisticallySignificant) {
        // Check for regressions first
        if (timeImprovementPercent < -REGRESSION_THRESHOLD || memoryImprovementPercent < -REGRESSION_THRESHOLD) {
            String explanation = String.format(
                "Performance regression detected: time %.1f%%, memory %.1f%%",
                timeImprovementPercent * 100, memoryImprovementPercent * 100);
            return new PerformanceResult(timeImprovementPercent, memoryImprovementPercent, 
                                       Status.FAIL, explanation, statisticallySignificant);
        }
        ...
        if (timeImproved || memoryImproved) {
            String explanation = String.format(
                ...
                timeImprovementPercent * 100, memoryImprovementPercent * 100);
            return new PerformanceResult(timeImprovementPercent, memoryImprovementPercent,
                                       Status.PASS, explanation, statisticallySignificant);
        }
        ...
        if (!statisticallySignificant) {
            String explanation = String.format(
                ...
                timeImprovementPercent * 100, memoryImprovementPercent * 100);
            return new PerformanceResult(timeImprovementPercent, memoryImprovementPercent,
                                       Status.WARNING, explanation, statisticallySignificant);
        }
