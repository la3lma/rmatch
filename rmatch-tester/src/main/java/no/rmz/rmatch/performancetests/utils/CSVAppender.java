...
public final class CSVAppender {
  ...
  /**
   * Append a logline to a file. If the file on is trying to write to does not exist, one is crated,
   * a line containing the text "secondsSinceEpoch, millisDuration, memoryInMb" is written to it,
   * and then a CSV line of values is written.
   *
        ...
        try {
          ...
          if (fileWriter != null) {
            ...
            throw new RuntimeException(
                "Something weird happening.  The file " + filePath + " resulted in a null pointer");
          }
        } catch (IOException ex) {
