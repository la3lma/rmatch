...
/**
 * Main class for running GitHub Action performance tests from command line.
 * This orchestrates the full performance comparison and generates results for CI.
 */
public final class GitHubActionPerformanceTestRunner {

    private static final Logger LOG = Logger.getLogger(GitHubActionPerformanceTestRunner.class.getName());
    ...
    private static void generatePerformanceReport(GitHubActionPerformanceTest.ComparisonResult result) {
        try {
            // Create detailed JSON report for artifacts
            StringBuilder jsonReport = new StringBuilder();
            jsonReport.append("{\n");
            jsonReport.append("  \"timestamp\": \"").append(java.time.Instant.now().toString()).append("\",\n");
            jsonReport.append("  \"performance_result\": {\n");
            jsonReport.append("    \"status\": \"").append(result.getPerformanceResult().getStatus().name()).append("\",\n");
            jsonReport.append("    \"explanation\": \"").append(result.getPerformanceResult().getExplanation()).append("\",\n");
            jsonReport.append("    \"time_improvement_percent\": ").append(result.getPerformanceResult().getTimeImprovementPercent()).append(",\n");
            jsonReport.append("    \"memory_improvement_percent\": ").append(result.getPerformanceResult().getMemoryImprovementPercent()).append(",\n");
            jsonReport.append("    \"statistically_significant\": ").append(result.getPerformanceResult().isStatisticallySignificant()).append("\n");
            jsonReport.append("  },\n");
    ...
    private static void appendResultsStats(StringBuilder sb, List<MatcherBenchmarker.TestRunResult> results, String indent) {
        if (results.isEmpty()) {
    ...
    private static void generatePRSummary(GitHubActionPerformanceTest.ComparisonResult result) {
        try {
            ...
            markdown.append("## ").append(perfResult.getStatus().getDisplayName())
            ...
            if (!result.getRmatchResults().isEmpty()) {
                double currentAvgTime = result.getRmatchResults().stream()
                    .mapToLong(r -> r.durationInMillis()).average().orElse(0);
                double currentAvgMemory = result.getRmatchResults().stream()
                    .mapToLong(r -> r.usedMemoryInMb()).average().orElse(0);
                
                String timeImprovement = String.format("%.1f%%", perfResult.getTimeImprovementPercent() * 100);
                String memoryImprovement = String.format("%.1f%%", perfResult.getMemoryImprovementPercent() * 100);
                
                markdown.append("| **Execution Time** | ").append(String.format("%.0f ms", currentAvgTime))
                        .append(" | Baseline | ").append(timeImprovement).append(" |\n");
                markdown.append("| **Memory Usage** | ").append(String.format("%.0f MB", currentAvgMemory))
                        .append(" | Baseline | ").append(memoryImprovement).append(" |\n");
                markdown.append("| **Statistical Significance** | ")
                        .append(perfResult.isStatisticallySignificant() ? "✅ Yes" : "⚠️ Low")
                        .append(" | - | - |\n");
            }
            
            markdown.append("\n### 🔬 Test Configuration\n");
            markdown.append("- **Runs**: ").append(result.getRmatchResults().size()).append(" iterations\n");
            markdown.append("- **Environment**: GitHub Actions (ubuntu-latest)\n");
            markdown.append("- **Java Version**: ").append(System.getProperty("java.version")).append("\n");
            ...
            markdown.append("\n---\n");
            markdown.append("*Performance check powered by [rmatch automated testing](PRD_PERFORMANCE_GITHUB_ACTION.md)*\n");
