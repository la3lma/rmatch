 ...
 * of Emily Brontes Wuthering Heights.
 */
public final class BenchmarkLargeCorpus {
   ...
   *     number of regexps to use. The second parameter is the path to the list of the regular
   *     expressions, the remaining parameters are the names of the files to be searched through
   *     using the regexps.
  ...
  public static void main(final String[] argv) throws RegexpParserException {
    ...
    if (argv.length < 4) {
      System.err.println(
          "Not enough parameters. Require at least 3: no of regexps, file where regexps are stored, series-ID (or empty string if auto-generated) one or more files to search through");
      System.exit(1);
    }
    ...
    System.out.println(
        "About to match "
            ...
            + nameOfRegexpFile
            + "'"
            + " then match them against a bunch of files");
    ...
    //           -- Do that using a matcher that builds up a structure that can be used to compare
    // results
    //              after the runs.
  ...
  private static void reportResult(
      String logfile,
      int actualNoOfRegexps,
      String testSeriesId,
      String corpusString,
      TestRunResult rmatchResult,
      TestRunResult javaResult) {
    ...
    final MatchPairAnalysis analysis =
        new MatchPairAnalysis(
            testSeriesId,
            metadata,
            actualNoOfRegexps,
            corpusString.length(),
    ...
    //       difference to the one not matching exists in the other set (the original set). Also see
    // which
    //       matches _do_ exist for the interval being covered by the mismatched element and report
    // that.
    //       The hypotheses are that there is either an alignment problem, a dominance problem, or
    // simply that
    //       the expression itself wasn't encoded in the (rmatch) matcher.  We need to eek out these
    // issues
