 ...
 * corpus and the set of regular expressions based on words from that corpus.
 */
public final class MatcherBenchmarker {
  /** Our wonderful log. */
  private static final Logger LOG = Logger.getLogger(MatcherBenchmarker.class.getName());
  ...
  /**
   * Set up the test by inhaling the corpus, enabling the regexpss and running the match.
   *
  ...
  public record TestPairSummary(
      ...
      String testSeriesId,
      String metadata,
      String matcherTypeName1,
      long usedMemoryInMb1,
      long durationInMillis1,
      String matcherTypeName2,
      long usedMemoryInMb2,
  ...
  private static String convertToCsv(TestPairSummary summary) {
    ...
    return Stream.of(data).map(c -> escapeSpecialCharacters(c)).collect(Collectors.joining(","));
  }

  public static final Comparator<LoggedMatch> matchComparator =
      new Comparator<MatcherBenchmarker.LoggedMatch>() {
        @Override
  ...
  public static void testMatcher(final Buffer b, final Matcher matcher, final String[] argv)
    ...
    testACorpus(b, matcher, noOfRegexps, REGEXP_LOCATION, RESULT_LOG_LOCATION);
  }
